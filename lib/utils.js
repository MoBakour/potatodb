...
/**
 * Performs projection (inclusion or exclusion of object fields based on projection object).
 *
 * If fields in the projection object were set to 1, then all other fields that are
 * not set to 1 will be removed from the target object.
 *
 * If fields in the projection object were set to 0, then these fields will be removed
 * from the target object.
 * @param {Object} obj Object to perform projection on
 * @param {Object} projectionObject Projection object
 * @returns {Object} The projected object
 */
function projectObject(obj, projectionObject) {
    if (!projectionObject) return obj;

    obj = deepCopy(obj);
    let excludes = true;

    for (let key of Object.keys(obj)) {
        if (projectionObject[key] === 0) excludes = false;
    }

    for (let key of Object.keys(obj)) {
        if (projectionObject[key] === 0) {
            delete obj[key];
        }

        if (projectionObject[key] === undefined && excludes) {
            delete obj[key];
        }
    }
...
module.exports = { extractNestedProperty, deepCopy, deepEqual, projectObject };
