/**
 * Gets the value of a property inside an object even if it was nested with the help of string paths.
 *
...
function selectFields(obj, selectionObject) {
    ...
    for (let key of Object.keys(obj)) {
        if (selectionObject[key] === 1) includes = false;
    }
    ...
    for (let key of Object.keys(obj)) {
        ...
        if (selectionObject[key] === undefined && !includes) {
            delete obj[key];
        }

        if (selectionObject[key] && typeof selectionObject[key] === "object") {
            obj[key] = selectFields(obj[key], selectionObject[key]);
        }
    }
