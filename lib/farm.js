...
class Farm {
    ...
    // PRIVATE METHODS
    /**
     * Private - Gets the value of a nested object property using a string path.
        ...
        if (typeof test === "object") {
            ...
            test = (potato) => {
                ...
                for (let key in query) {
                    const extractedData = Farm.#getFromNested(potato, key);
    ...
    async #updateLogic(test, updates, updated = true, caller) {
        ...
        try {
            ...
            function operatorAction(update, index) {
                ...
                for (let key of keys) {
                    let target = Farm.#getFromNested(data[index], key);

                    switch (update) {
                        case "$inc": {
                            target += updates[update][key];
                            break;
                        }
                        case "$push": {
                            target.push(updates[update][key]);
                            break;
                        }
                        case "$pull": {
                        ...
                        case "$concat": {
                            target = target.concat(updates[update][key]);
                            break;
                        }
                    }
