...
class Farm {
     ...
     * @param {boolean} id Specifies whether potatoes (documents) in
     *      this farm should be stamped with id strings or not.
     * @param {boolean} timestamps Specifies whether potatoes (documents) in this farm
     *      should be stamped with timestamps or not.
     */
    constructor(farmName, farmPath, dbName, id, timestamps) {
            ...
            test = (potato) => {
                ...
                for (let key in query) {
                    ...
                    if (key.startsWith("$")) {
                        ...
                        matching.push(query[key].test(extractedData));
                        ...
                        matching.push(query[key] === extractedData);
                    }
                }
    ...
    static #preInterceptor(data, options) {
        ...
        // skipping
        data = skip < 0 ? data.slice(0, data.length + skip) : data.slice(skip);
    ...
    /**
     * Post interceptor applied some options to the reults after query is done.
     *
     * @private
     * @static
     * @async
    ...
    /**
     * Counts the number of potatoe documents stored in the farm (collection) file.
     *
     * @async
     * @returns {Promise<number>} The number of stored potatoe documents in farm.
     */
    async countPotatoes() {
        try {
    ...
    async #findLogic(caller, test, options) {
        try {
            ...
            if (caller === "findOne") {
                ...
                result = new PotatoArray(...result);
            } else throw new PotatoError("Invalid caller");

            // post interceptor
            return await Farm.#postInterceptor(result, options, caller);
        } catch (err) {
            throw new PotatoError(err.message);
        }
    }
    ...
    async #updateLogic(caller, test, updates, options) {
        try {
            ...
            return await Farm.#postInterceptor(results, options, caller);
        } catch (err) {
            throw new PotatoError(err.message);
        }
    }
    ...
    async #deleteLogic(caller, test, options) {
        ...
        try {
            ...
            if (Array.isArray(results)) {
                results = results.map((obj) => selectFields(obj, select));
                results = new PotatoArray(...results);
            } else {
                results = selectFields(results, select);
            }
